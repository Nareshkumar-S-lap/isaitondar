...
export default function CreateThevaram() {
  const { user, hasRole } = useAuth();
  const { addPathigam } = useData();
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-2">You don't have permission to create Thevaram entries.</p>
          <Button 
            variant="secondary" 
  ...
  const onSubmit = async (data: ThevaramFormData) => {
    try {
      ...
      addPathigam(newPathigam);
      toast.success('Thevaram entry created successfully!');
      navigate('/thevaram');
    } catch (error) {
      toast.error('Failed to create Thevaram entry');
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Create Thevaram Entry</h1>
          <p className="text-gray-600">Add a new sacred hymn or pathigam</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="space-y-6">
            <FormField label="Tamil Content" required>
              <RichTextEditor
                value={contentTamil}
            ...
            <FormField label="Transliteration" required>
              <RichTextEditor
